...
async function insertEmployee(newEmployee) {
  ...
  try {
    ...
    if (
      error.code === "23505" &&
      error.constraint === "employees_contact_phone_key"
    ) {
...
async function editEmployeeInfo(employeeExtras, educationArray, id) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query(
      `UPDATE employees
                SET professional_title = $1, bio = $2, portfolio_url = $3
                WHERE user_id = $4;`,
      [
        employeeExtras.professional_title,
        employeeExtras.bio,
        employeeExtras.portfolio_url,
        id
      ]
    );
    ...
    educationArray.forEach((edu, index) => {
      ...
      valuesClause += `($${offset + 1}, $${offset + 2}, $${offset + 3}, $${
        offset + 4
      }, $${offset + 5})`;
      queryParams.push(
        id,
    ...
    if (educationArray.length > 0) {
      await client.query(
        `INSERT INTO education (user_id, degree_name, institution, start_year, end_year)
            VALUES ${valuesClause}`,
        queryParams
      );
    }
    ...
    client.release(); // Added client release in finally block
  }
}
